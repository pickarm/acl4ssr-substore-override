...
async function smokeTest(jsText, meta, providers) {
...
async function main() {
  ...
  await smokeTest(js, meta, providers);
  ...
  await writeFile('upstream.json', JSON.stringify({ upstream: UPSTREAM, sha256: upstreamSha, generatedAt: new Date().toISOString(), ...meta, mirroredRulesets }, null, 2) + '\n', 'utf8');
  console.log(`Generated ${OUT}: ${meta.rules} rules, ${meta.groups} groups, ${meta.providers} mirrored providers`);
}
